from unihiker_k10 import screen, button
import time
...
PAGE1 = [
    ...
    ("Pregunta 1",       SURVEY_CLR),
    ("Pregunta 2",       SURVEY_CLR),
    ("Pregunta 3",       SURVEY_CLR),
]
...
# ---------------------------------------------------------------------------
def move_up():
    """Move selection one row up; wrap to previous page or last row of last page."""
    state["selected"] -= 1
    if state["selected"] < 0:
        state["page"] = (state["page"] - 1) % TOTAL_PAGES
        state["selected"] = ITEMS_PER_PAGE - 1
...
# ---------------------------------------------------------------------------
# Button callbacks  —  A = UP  |  B = DOWN
# ---------------------------------------------------------------------------
bt_a = button(button.a)
bt_b = button(button.b)

bt_a.event_pressed = lambda: move_up()
bt_b.event_pressed = lambda: move_down()
