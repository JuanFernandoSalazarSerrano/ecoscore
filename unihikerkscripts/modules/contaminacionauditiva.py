...
def draw_header():
	...
	screen.draw_text(text="AUDITIVA", x=126, y=7, font_size=13, color=0x3D8F75)
...
def draw_logo_mark(cx=120, cy=74):
	screen.set_width(width=3)
	screen.draw_circle(x=cx, y=cy, r=22, bcolor=EMERALD)
	screen.draw_circle(x=cx, y=cy, r=18, bcolor=LIME, fcolor=LIME)
	screen.draw_circle(x=cx, y=cy, r=13, bcolor=WHITE, fcolor=WHITE)

	screen.set_width(width=2)
	screen.draw_circle(x=cx - 6, y=cy + 3, r=6, bcolor=EMERALD, fcolor=EMERALD)
	screen.draw_circle(x=cx + 7, y=cy - 5, r=7, bcolor=DEEP_GREEN, fcolor=DEEP_GREEN)
	screen.draw_circle(x=cx + 2, y=cy + 8, r=5, bcolor=TEAL, fcolor=TEAL)
	screen.draw_line(x0=cx - 8, y0=cy + 12, x1=cx + 10, y1=cy - 7, color=WHITE)
	screen.set_width(width=1)


def draw_subtext():
	screen.draw_text(text="Grabe el sonido ambiental", x=34, y=80, font_size=10, color=0x3D8F75)
