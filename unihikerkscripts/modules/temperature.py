...
def draw_header(unit_label):
	...
	screen.draw_text(text="TEMP " + unit_label, x=82, y=84, font_size=10, color=TEAL)
...
def run():
	unit = state.get("temp_unit", "C")
	unit_label = "C" if unit != "F" else "F"

	draw_background()
	draw_card()
	draw_header(unit_label)
	draw_value_box()
	...
	while True:
		...
		if unit_label == "F":
			...
			value = temp_c
		draw_value_box()
		screen.draw_text(text="T: " + str(value), x=55, y=152, font_size=18, color=DEEP_GREEN)
		screen.show_draw()
		for _ in range(10):
			if utils.pressed(btn_a):
				return
